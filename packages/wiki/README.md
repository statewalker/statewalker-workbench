...
- **GraphExtractor is disabled** in the default pipeline — its per-section `graph.json` is a leaf artifact nothing downstream consumes. `graphBuilder` / `WikiPageGraph` remain (still unit-tested) for re-enabling.
