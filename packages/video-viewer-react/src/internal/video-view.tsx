...
/**
 * Bound to the `video-viewer` catalog's `VideoView` component type.
 * Lazy-loads the file via `files:load-file` on mount, builds a
 * `blob:` URL using the loaded MIME type (or `video/mp4` as a
 * sensible default), and renders a `<video controls>` element. Revokes
 * the URL on unmount or URI change.
 */
      ...
      .promise.then((loaded) => {
        ...
        const url = URL.createObjectURL(
          new Blob([loaded.bytes as BlobPart], {
            type: loaded.mimeType ?? "video/mp4",
          }),
        );
